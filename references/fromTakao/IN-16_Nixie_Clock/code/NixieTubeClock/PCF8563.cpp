...
DateTime::DateTime(uint16_t year, uint8_t month, uint8_t day, uint8_t dayOfTheWeek, uint8_t hour, uint8_t minute, uint8_t second){
    _y = year;
    _mon = month;
    _day = day;
    _h = hour;
...
void PCF8563::adjust(DateTime now){
    setTime(now);
}
