 ...
 *
 */
String Button::info2String_short(ButtonInfo_t *info, bool interrupted) {
  char buf[64];
  String intrString = interrupted ? "!" : " ";
  String valueString = info->value ? "H" : "L";
  String longPressedString = info->long_pressed ? "L" : "-";
  
  sprintf(buf, "%s%s %s P%d C%d %s R%d",
          intrString.c_str(),
          info->name,
          valueString.c_str(),
          info->push_count, info->click_count,
 ...
 *
 */
String Button::toString_short(bool interrupted) {
  return Button::info2String_short(&(this->info), interrupted);
} // Button::toString()
